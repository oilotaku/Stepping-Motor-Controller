...
## 第四輪：2026-08-26～31 實機事故修正與模組化（2026-09-03 補記）

本節是 `reporter` 代理依 HANDOVER.md〈文件同步缺口〉一節的要求，事後彙整既有 commit history／`docs/fiber-scan.md`／`docs/modularization.md`／HANDOVER.md 內容補上的落差項目，**不是新的設計判斷**——所有結論都轉述自上述文件，沒有一項是本節自己下的架構或數值判斷。上面〈2026-08-26～2026-08-31 後續更新〉一節（2026-09-02 補記）已經涵蓋前五項的大部分內容；本節在同一批項目上補全「main_ai.py 模組化複審」這個先前遺漏的落差，並把七個項目的驗證狀態統一列表，方便一眼核對。

### 驗證狀態總表

| 項目 | 日期 | 驗證狀態 | 詳細記錄 |
|---|---|---|---|
| 1. 階段零盲搜（`run_stage0_blind()`）+ 觸發判準兩次修正 | 2026-08-26 | 假物件測試（`verify_blind_scan.py` 44 項）+ log 觀察佐證，**未完整實機驗證** | [docs/fiber-scan.md](docs/fiber-scan.md)〈階段零：盲搜粗掃＋光功率計量程〉；上方本文件〈階段零：無光位置盲搜〉 |
| 2. 光功率計量程修正（`meter_GPIB.py` 自動量程＋sentinel 退回） | 2026-08-26 | 假物件測試 + **實機 log 佐證讀值間歇性**（`σ=0.46271，基準功率 -57.2439 dBm`），量程自動退回本身未完整驗證 | 同上 |
| 3. 撞限位反覆撞修正（`_note_limit_hit()`/`_targets_reachable()`）+ 階段一雜訊門檻 | 2026-08-26～27 | **部分實機驗證**（2026-08-27，COM2，Y 軸，僅涵蓋 `_move_relative()` 單軸路徑）；`_move_multi_axis()` 多軸路徑與跨 `run()` 的 `_travel_bounds` 重置仍只有假物件驗證 | [docs/fiber-scan.md](docs/fiber-scan.md)〈撞限位不再反覆撞〉 |
| 4. 尋光樣本 Excel 報表匯出 | 2026-08-26～27 | 假物件測試（`verify_scan_export.py` 24 項，真的寫檔後用 `zipfile` 解開驗證），**無實機驗證**（不需要，純檔案 I/O 功能） | [docs/fiber-scan.md](docs/fiber-scan.md)〈尋光樣本的 Excel 報表〉 |
| 5. 殘差診斷與收尾曲率擬合微調（`08e0bcd`） | 2026-08-31 | 假物件測試（`verify_fiber_scanner_signal.py` 新增 22 項），**`enable_curvature_fit` 預設關閉，未真機驗證前不建議開啟** | 上方本文件〈尋光演算法殘差診斷與收尾曲率擬合微調〉；`git show 08e0bcd` |
| 6. main_ai.py 模組化正式複審 + `LongOperation`/`PowerReading` | 2026-08-31 | 架構複審（非數值/演算法判斷）+ 回歸測試 413→421 項零破壞，**與硬體行為本身無關，不適用「實機驗證」這個標籤** | [docs/modularization.md](docs/modularization.md)〈2026-08-31 重新評估〉〈六〉～〈九〉 |
| 7. `gaussian_vector_sim.py` / `gaussian_vector_sim_gui.py` | 2026-08-28～31 | 純數學模擬工具，**完全不連硬體，明確不能算真機驗證的替代品** | [docs/fiber-scan.md](docs/fiber-scan.md)；上方本文件〈多軸高斯向量圖模擬工具〉 |

項目 1～5、7 上方〈2026-08-26～2026-08-31 後續更新〉一節已有對應段落，此處不重複展開內容，只用這張表統一標註驗證狀態。第 6 項是先前兩次更新（2026-09-02、以及本節開頭修訂的檔頭說明）都遺漏的落差，補充如下。

### main_ai.py 模組化正式複審（2026-08-31）——與尋光相關的部分

`architect` 對照 [docs/modularization.md](docs/modularization.md) 既有的四條門檻做正式複審（觸發原因：main_ai.py 行數達 6820 行，越過門檻 4）。**結論是「仍不拆分頁 mixin」**——完整理由與四條門檻的逐條判定在 [docs/modularization.md](docs/modularization.md)〈2026-08-31 重新評估〉〈一〉〈二〉，本節不重複，只記與尋光功能直接相關、會影響下一個要碰尋光程式碼的人的部分：

- **複審過程中額外抓到三項散彈式接線安全缺陷**，其中至少兩項直接影響尋光的可停止性：**尋光執行中按 Esc 或「■ Stop」滑台會停一下又自己繼續走**、**尋光/重播執行中按頂列「中斷」背景執行緒會繼續對已關閉的序列埠送指令**。第三項（`_do_stop` 對重播的行為與 `_on_escape` 不一致）不直接涉及尋光。三項已於同日全部修正（`docs/modularization.md`〈六〉）。
- **`LongOperation` 註冊表**（`docs/modularization.md`〈七〉）：把「長時間背景作業」（含尋光）的 UI 忙碌顯示與停止請求分派收斂成單一註冊表 `self._long_ops`，取代先前要作者手動記得接進 6 個位置（`_update_stat_ui`／`_start_poller`／`_do_stop`／`_on_escape`／`_toggle_connect`／`_on_close`）的舊模式——CLAUDE.md 已記載這正是上面漏接三項安全缺陷的根本原因。**新增任何尋光相關的長時間背景作業一律只在 `_register_long_ops()` 註冊一筆，不要再手動接六個位置**；註冊表只服務 UI 忙碌顯示與停止分派，明確不可把 `motion_active`／`scanning_active` 併進來當移動守衛。
- **`PowerReading` frozen dataclass**（`docs/modularization.md`〈九〉）：「光功率」與「尋光」兩個分頁原本各自直接寫入三個裸欄位記錄「最近一次讀值」，收斂成單一 frozen dataclass，`_scan_plot_extend()`（尋光即時軌跡圖的資料寫入點）的 7 個分散寫入點收斂成 1 個呼叫。frozen 是唯一真正的技術理由——讓移動執行緒對這份跨執行緒共享狀態的讀取變成單一次屬性重新指派、避免撕裂讀取，**文件本身明確標註不宣稱新增任何鎖保護**，讀者不要誤讀成這是一次上鎖修正。
- 回歸測試 413→421 項，`ui_theme.py` 那一步零修改零破壞；main_ai.py 行數從 6820 漲到 7114（+294，複審記錄註明幾乎全是機制與說明註解，非邏輯膨脹）。**這次複審自己下的結論是「行數對『該不該拆』沒有預測力」**，CLAUDE.md 已把這句話收進正文，此處不再另外判斷。
