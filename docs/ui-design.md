...
- **主視窗是最大化開機（`state("zoomed")`），所以任何「開在主視窗右緣／下緣外側」的 `Toplevel` 定位都會落在螢幕外。** 2026-08-26 使用者回報「浮動視窗全螢幕下會消失」即此：`x = root.winfo_x() + root.winfo_width() + 10`，最大化時實測 root 在 `(-8, -8)`、寬 1920，算出 x=1922 > 螢幕寬 1920，視窗確實被建立了、只是看不到（原本的註解還寫著「簡單且不會跑到螢幕外」，正好寫反）。定位邏輯一律要有「放不下就退回主視窗內側」的分支，最後再把 x／y 夾回螢幕範圍。實作見 `DS102GUI._pm_float_position()`——刻意寫成不碰 tkinter 的純函式，才能用假尺寸驗證各種螢幕組合（`verify_meter_panel.py::TestPmFloatPosition`，6 項）。多螢幕的實際可視範圍 tkinter 問不到（`winfo_screenwidth()` 只回主螢幕），這裡取保守解：問不到就放進主視窗內側，最差是位置不理想，但不會消失。
