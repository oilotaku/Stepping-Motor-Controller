...
1. ~~長按點動的 Python 端保護預設不生效~~——**2026-09-02 已部分修正**，見 [docs/threading.md](threading.md)〈長按點動的限位保護〉。`connect()` 現在會呼叫 `sync_sw_limits_from_controller()` 把韌體端限位鏡射進 `self.sw_limits`（只收緊、永不放寬、永不清空），並在兩層都沒保護時跳警示橫幅。**但這治不好「出廠狀態下沒有保護」本身**——韌體軟體限位出廠／斷電就是停用的，同步邏輯查到的是哨兵值，`sw_limits` 該側依然是 `None`。真正讓長按點動預設就有保護，需要使用者主動設定韌體限位並用〈儲存控制器設定〉持久化，或每次連線後手動套用 `sw_limits`（後者本身尚未持久化，刻意排除在這次修法之外）。假物件驗證（`verify_sw_limit_sync.py`，15 項）。

   **2026-09-02 實機驗證通過（COM，X 軸，CW 限位 8000 pulse）**：`sw_limits` 有值時 `_watch_jog_limit()` 確實正確攔截長按點動，多次連續 CW 點動全部在越過 8000 之前被 `STOP` 擋下，未曾滑出限位。同時觀察到一個純物理現象、非程式錯誤——目前這組點動速度設定（`F0=5000, R0=1000`）換算出的剎車距離 `F0×R0/2000=2500 pulse`，加上查詢週期的提前量，會讓監看執行緒在離限位還有約 2400～3000 pulse 時就先停（佔 8000 pulse 限位範圍的 3 成以上）。這是 `_watch_jog_limit()` 公式（見其 docstring）「寧可提早停也不要越界」的設計本身，不是本次改動引入的行為，只是這是它第一次在真機上被真的觸發（過去 `sw_limits` 全空，這段程式碼從未真的跑過）。若這個緩衝範圍在實務上造成困擾（可用行程被大幅犧牲），可考慮調低〈移動控制〉分頁的點動速度預設值縮短剎車距離——**這次驗證的結論是「機制正確、沒有滑出限位」，速度參數本身要不要調整是獨立的後續事項，本次未動**。
