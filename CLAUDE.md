...
| `AXI{n}:DRDIV?` | 驅動器分割(0=full step…15=1/250)。**查證來源：`ds102 (2).pdf` 第 131 頁〈Inquiry Command〉表**，官方指令名 `:DRiverDIVision?`/`:DRDIV?`，不是猜的。手冊註記「Only MS driver effective」——0～15 完整對照表只在驅動器是 **MS（Microstep）型**時才有意義，Normal 型驅動器只有 0/1 兩態有效。2026-08-18 實機確認**這台滑台用的是 AMS（微步進）型驅動器**，所以完整的 0～15 細分對照表有意義。2026-08-18 起 `connect()` 會查一次存進 `ctrl.axis_drdiv: Dict[str, str]`，GUI 頂部與 LOG 顯示原始值。🔴 **純資訊性顯示，沒有拿它做任何 pulse→um 換算**——換算還需要螺桿導程等機械參數（目前未知），而且同樣受 `RESOLUT?`=1（未配置真實尺度）這個既有問題影響，貿然換算等於重蹈 2026-08-05 拿掉 um/mm 單位切換的覆轍。要做換算前**先跟使用者確認機械參數或用實測位移反推**，不要只憑 DRDIV 自己算 |
