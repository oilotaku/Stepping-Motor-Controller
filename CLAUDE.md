...
| [sim/gaussian_vector_sim.py](sim/gaussian_vector_sim.py) / [sim/gaussian_vector_sim_gui.py](sim/gaussian_vector_sim_gui.py) | **2026-08-28～31 新增**：多軸高斯耦光場數學模擬（CLI＋獨立 tkinter GUI），逐行對照 `fiber_scanner.py` 的 `run_stage0_blind()` / `run_stage1()` / `_search_axis_once()` / `_rect_spiral_offsets()` 移植，用來在沒有硬體時觀察梯度場長相與驗證尋光演算法行為、調參。搜尋邏輯必須忠實重現 `fiber_scanner.py`，不可另外設計通用演算法（早期 steepest descent + Rprop 版本因與真實座標下降是兩套邏輯而重寫）；真實梯度只用來畫背景向量圖，搜尋本身跟真機一樣只靠「移動＋量測」的有限差分。完全獨立於 main_ai.py，**不 import 任何 DS102 相關模組**，不連接硬體。**2026-09-09 修正 GUI 端 3 軸模擬讀不到數值的問題**：`gaussian_vector_sim_gui.py` 原本只有一組全軸共用的「繪圖範圍下限／上限」，多軸時各軸中心與 σ 尺度差很大（例如 Z 軸對焦行程遠比 X/Y 橫向對準行程長）時，共用同一組範圍會讓格點解析度完全罩不住尺度較小的軸——使用者實測回報「3 軸模擬時 X-Y 讀不到數值」，重現後確認即使 3 個軸對子圖全部 0/625 可讀（不是只有 X-Y）。改成比照 `gaussian_vector_sim.py` 的 `--grid-range` 每軸各自一組 `(min, max)`，欄位加進「模擬軸」表格（`_axis_rows` 每列多兩個 `range_min`/`range_max` 輸入框），移除原本共用的 `_grid_min`/`_grid_max` |
...
- **`ctrl.check_sw_limits_batch()` 在實機出廠狀態下預設是 no-op**（`sw_limits` 六軸都是 `(None, None)`，韌體 `CWSLE`/`CCWSLE` 出廠停用且 RAM-only）。**2026-09-02 起連線時會呼叫 `sync_sw_limits_from_controller()` 把韌體端限位鏡射進 `sw_limits`**，但這只在使用者已經設定並啟用過韌體限位（且用〈儲存控制器設定〉持久化過）時才有值可鏡射——出廠狀態下同步邏輯查到的是哨兵值，`sw_limits` 依然全 `(None, None)`，no-op 這件事本身沒有改變。尋光的行程保護**只能靠撞過一次之後記住的 `_travel_bounds`**，不可假設 `sw_limits` 預設有值。新增任何「送出前檢查行程」的程式碼一律走 `_targets_reachable()`，不要只呼叫 `check_sw_limits_batch()`。（fiber-scan、threading）
